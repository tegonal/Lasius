...
export const getUpdateUserPasswordMutationFetcher = (
  ...
  return (_: Key, { arg }: { arg: ModelsPasswordChangeRequest }): Promise<null> => {
...
export const oauthAccessToken = (options?: SecondParameter<typeof lasiusAxiosInstance>) => {
  return lasiusAxiosInstance<null>({ url: `/oauth2/access_token`, method: 'POST' }, options)
}
...
export type LoginMutationError = ErrorType<null>

/**
 * @summary login to the oauth provider with the authorization code flow
 */
export const useLogin = <TError = ErrorType<null>>(options?: {
  swr?: SWRMutationConfiguration<
...
export const logout = (options?: SecondParameter<typeof lasiusAxiosInstance>) => {
  return lasiusAxiosInstance<null>({ url: `/oauth2/logout`, method: 'POST' }, options)
}
